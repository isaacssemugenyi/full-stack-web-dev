...
const validations = [
    ...
    check('title')
        .trim().isLength({min: 3})
        .escape().withMessage('A title is required'),
    check('message')
        .trim().isLength({min: 5})
        .escape().withMessage('A message is required'),
];
...
module.exports = params => {
    ...
    router.get('/', async (req, res, next)=>{
        try {
            ...
            // checking if errors exist in the session object, then we can return them back to the feedback form for the user.
